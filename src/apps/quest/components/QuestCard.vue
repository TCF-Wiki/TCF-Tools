<template>
    <section ref="card" class="card" @click="openModal()">
        ...
        <div class="card__button" role="button" @click.stop.prevent="progressInfo.toggle(name)">
            <div class="card__button-text" :class="faction">
                <svg v-if="progressInfo.get()[name] >= missionListData[name]['missions'].length" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 448 512">
        ...
        <div class="card__contents">
            ...
            <div class="card__parts" v-if="parts">{{ progressInfo.get()[name] }} / {{ missionListData[name]['missions'].length }}</div>
        </div>
    </section>
        ...
        <Transition name="modal">
            <div class="modal__bg" v-if="isModalOpen">
                <section class="modal__content" ref="modal">
                    ...
                    <QuestParts :name="name" :faction="faction" />
                </section>
            </div>
        </Transition>
    </Teleport>
...
export default defineComponent({
    ...
    mounted() {
        ...
        if (mission) {
            ...
            this.parts = `${this.progressInfo.get()[this.name]} / ${mission['missions'].length}`;
        }
    },
    methods: {
        addShake() {
