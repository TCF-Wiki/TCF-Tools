import { weaponData, targetData, armorData, itemData } from "./data";
import {
  selectedTarget,
  selectedArmor,
  selectedAccuracy,
...
import { ammoStackBuySize, roundToOne } from "./utils";
...
export function updateRunTimeSettings() {
  ...
  runTimeSettings.shotsToKillTableBody.weakspotValue =
    selectedWeakspotValue.value;

  runTimeSettings.shotsToKillTableHead.distance = selectedDistance.distance;
  runTimeSettings.shotsToKillTableHead.weakspotValue =
    selectedWeakspotValue.value;
...
export const calculate = {
  damagePerMag: function () {
    return this.totalClicksPerMag() * this.totalDamagePerClick();
  },
  ...
  roundsPerMinuteReloadAdjusted: function () {
    ...
    const roundsPerMagazine = this.roundsPerClick() * this.totalClicksPerMag();
    const magazinesPerMinute =
      60 / (this.totalTimeToEmptyMag() + this.s("reloadTime"));
  ...
  damagePerSecondReloadAdjusted: function () {
    ...
    return (
      this.damagePerMag() / (this.totalTimeToEmptyMag() + this.s("reloadTime"))
    );
  },
  ...
  totalTimeToEmptyMag: function () {
    ...
    return (
      this.totalTimePerClick() * this.totalClicksPerMag() - this.s("refireTime")
    );
  },
  ...
  timeToKill: function () {
    ...
    if (remainingBurstBullets) {
      remainingBurstTime =
        this.s("spinupTime") +
        remainingBurstBullets * this.s("burstInterval") -
  ...
  ammoCostPerKill: function () {
    const ammoType = this.s("ammoType");
    const costOfOneBullet =
      itemData["Ammo_" + ammoType]["cost"][0]["SoftCurrency"] /
      ammoStackBuySize[ammoType];
    const amountOfBullets = this.shotsToKill() * this.s("ammoPerBullet");

    return amountOfBullets * costOfOneBullet;
  },
  ...
  creatureDamageMult: function () {
    if (settings.target != "PlayerDefault") {
      if (settings["creatureDmgMult"] != null)
        return settings["creatureDmgMult"];
